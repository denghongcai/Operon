...
for module in runtime fs exec service policy audit discovery trace; do
  test -f "crates/operon-core/src/${module}.rs"
  rg -n "pub mod ${module};" crates/operon-core/src/lib.rs
done

for reexport in audit discovery fs exec policy runtime service trace; do
...
rg -n 'pub struct ExecRecord' crates/operon-core/src/exec.rs

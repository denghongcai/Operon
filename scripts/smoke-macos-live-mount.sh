...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "macOS live mount smoke requires macOS" >&2
  exit 1
fi
...
MOUNT_NAME="operon-live-mount-macos-live-mount-$$"
  ...
  nodes:
    macos-live:
      ...
      auth:
        token: macos-live-token
policy:
  subject: live-mount-macos-live-smoke
  fs:
    mounts:
      - name: workspace
        path: /
...
echo "macOS live mount smoke passed"

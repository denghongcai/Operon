...
for node in node-a node-b; do
  ...
  cargo run -q -p operon-cli -- --json --config examples/docker-nodes.yaml service list "$node" >/tmp/operon-"${node}"-services.json
  grep -q '"id": "daemon"' /tmp/operon-"${node}"-services.json
  cargo run -q -p operon-cli -- --config examples/docker-nodes.yaml service check "$node" daemon
  cargo run -q -p operon-cli -- --json --config examples/docker-nodes.yaml service check "$node" daemon >/tmp/operon-"${node}"-service-check.json
  grep -q '"ok": true' /tmp/operon-"${node}"-service-check.json
  if cargo run -q -p operon-cli -- --config examples/docker-nodes.yaml service check "$node" missing >/tmp/operon-"${node}"-service-deny.log 2>&1; then
    echo "expected denied service policy failure for ${node}" >&2
    exit 1
  fi
  cat /tmp/operon-"${node}"-service-deny.log
  ...
  cargo run -q -p operon-cli -- --config examples/docker-nodes.yaml audit show "$node" --capability service:daemon --action check --allowed true --resource daemon --limit 5
  cargo run -q -p operon-cli -- --config examples/docker-nodes.yaml audit show "$node" --capability service:default --action check --allowed false --resource missing --limit 5
...
cargo run -q -p operon-cli -- --json trace show /tmp/operon-docker-trace.json >/tmp/operon-docker-trace-show.json
grep -q '"run_id"' /tmp/operon-docker-trace-show.json
cargo run -q -p operon-cli -- trace list /tmp
chmod -R u+w /tmp/operon-mount-poc 2>/dev/null || true
rm -rf /tmp/operon-mount-poc

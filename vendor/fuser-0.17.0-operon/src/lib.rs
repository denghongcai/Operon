...
/// On macOS, keep default capabilities conservative for FUSE-T's NFS bridge.
///
/// `FUSE_VOL_RENAME` and `FUSE_XTIMES` require matching filesystem operations,
/// and the bit used for `FUSE_VOL_RENAME` collides with Linux `FUSE_INIT_EXT`.
#[cfg(target_os = "macos")]
